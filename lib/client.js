...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** The tagline text; empty means "not configured" (the node is removed). */
		function taglineText(state) {
			return str(state, "heroTagline", "").trim();
		}

		/** The tagline alignment; the shipped centring unless the user picked otherwise. */
		function taglineAlign(state) {
			return oneOf(state, "taglineAlign", TAGLINE_ALIGNS, "center");
		}
		...
		function syncHeroTagline(state, shipped) {
			...
			// Not configured: remove the node so the shipped hero is untouched. This
			// is also the restore path when the user clears the field.
		...
		function copyActive(state) {
			return (
				...
				(enabled(state) && taglineText(state) !== "") ||
					...
					taglineText(state) !== ""
						? h(
								...
								h(Segmented, {
									...
									onChange: (value) => set({ taglineAlign: value === "center" ? "" : value }),
								}),
							)
						: null,
					h(
		...
		const ZH = {
			...
			"hero.hint": "首页标题与徽标不在插槽里，插件按当前语言读取官方文案后做等值替换；清空即恢复。标语是插件新增的一行，清空即移除；对齐默认居中，选择「与标题对齐」后标题左边缘动态对齐。",
			"browser.title": "浏览器",
			"browser.tab": "标签页标题",
