// dsh-custom-brand — browser half (client plugin bundle).
//
// Loaded by dsh-client-modules at /plugins/dsh-custom-brand/client.js and
...
//
// Preferences live in $DSH_HOME/custom-brand.json through the host half's
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SEED_KEY = "dsh.custom-brand.v1";
		/** Host persistence route (see lib/index.js). */
		const API_PATH = "/custom-brand/api";
		/** Locale namespace owned by this plugin. */
		const NS = "custom-brand";
		/** Settings-section identity in the settings navigation. */
		const SECTION_ID = "custom-brand";
			...
			const emit = () => {
				for (const listener of [...listeners]) {
					try {
						...
						console.error("[dsh-custom-brand] store listener failed", error);
					}
				}
			};
			return {
		...
		function writeSeed(state) {
			try {
				...
				console.warn("[dsh-custom-brand] could not seed localStorage", error);
			}
		}
		...
		function normalizeImage(dataUrl, done) {
			...
			image.onload = () => {
				try {
					...
					console.error("[dsh-custom-brand] image scaling failed", error);
					done(dataUrl);
				}
			};
			image.onerror = () => {
				console.error("[dsh-custom-brand] image could not be decoded");
				done(null);
			};
			image.src = dataUrl;
		}
					...
					reader.onload = () => {
						normalizeImage(String(reader.result), (dataUrl) => {
							...
							if (dataUrl.length > MAX_IMAGE_CHARS) {
								console.warn(
									`[dsh-custom-brand] image is ${dataUrl.length} characters; pick a smaller or simpler one`,
								);
		...
		function shippedCopy(ctx) {
			try {
				...
				console.warn("[dsh-custom-brand] shipped hero copy unavailable", error);
		...
		const ZH = {
			...
			"footer.hint":
				"配置保存在 $DSH_HOME/custom-brand.json（localStorage 只作首屏种子），刷新页面后侧栏与首页即生效。",
			pick: "选择图片",
			clear: "清除",
		};
		...
		const EN = {
			...
			"footer.hint":
				"Settings live in $DSH_HOME/custom-brand.json (localStorage is only the first-paint seed). Reload the page to see the sidebar and hero.",
			pick: "Choose image",
			clear: "Clear",
		};
		...
		function apply(ctx) {
			...
			ctx.effect(
				...
				"dsh-custom-brand: settings dictionaries",
			);
			...
			const persist = () => {
				...
				callHost({ method: "set", patch }).catch((error) =>
					console.warn("[dsh-custom-brand] settings not saved:", error.message),
				);
			};
			persist();
			ctx.effect(
				...
				"dsh-custom-brand: settings persistence",
			);
			callHost({ method: "get" })
				.then((value) => {
					hydrating = false;
				...
				.catch((error) => {
					hydrating = false;
					console.warn("[dsh-custom-brand] host settings unavailable, using browser seed:", error.message);
				})
				.finally(() => {
					persist();
				});
			...
			}, "dsh-custom-brand: brand occupants");
			...
			ctx.effect(
				...
				"dsh-custom-brand: settings page",
			);
			...
			const sync = () => {
				...
				try {
					...
					console.error("[dsh-custom-brand] brand sync failed", error);
				}
			};
			const schedule = () => {
			...
			ctx.effect(
				...
				"dsh-custom-brand: dom substitutions",
			);
