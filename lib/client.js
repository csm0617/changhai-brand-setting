...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * The tab title used when the field is left empty.
		 *
		 * Every other surface here ships a default and treats an empty field as
		 * "use it" — the mark, the favicon and the tagline all work that way. The
		 * title only differed because it has no sensible empty state of its own;
		 * now that it ships copy too, an empty field means the same thing it means
		 * everywhere else, and the shipped product name leaves the tab for good.
		 */
		const DEFAULT_TAB_TITLE = "长海智能";
		 ...
		 *
		 * An empty field falls back to the built-in title rather than to the shipped
		 * product name, matching the mark, the favicon and the tagline.
		 *
		 * `appliedTitle` remembers what this plugin last wrote. Without it the
		 * substitution is one-shot: the product name is gone after the first pass,
		 * so a later edit finds nothing to replace and the tab keeps the stale value
		 * — which is exactly what a fresh install followed by a settings edit did.
		 * Rolling our own value back first makes the rewrite idempotent.
		 */
		let appliedTitle = "";
		function syncTitle(state) {
			const replacement = str(state, "title", DEFAULT_TAB_TITLE) || DEFAULT_TAB_TITLE;
			// Undo a previous write so the product name is searchable again.
			let title = document.title;
			if (appliedTitle !== "" && title.includes(appliedTitle)) {
				title = title.split(appliedTitle).join(DEFAULT_PRODUCT_NAME);
			}
			if (!title.includes(DEFAULT_PRODUCT_NAME)) {
				// Someone else owns this title now; drop our record rather than
				// restoring into a title that has moved on.
				appliedTitle = "";
				return;
			}
			const next = title.split(DEFAULT_PRODUCT_NAME).join(replacement);
			if (next !== document.title) document.title = next;
			appliedTitle = replacement;
		}
		...
		/**
		 * Whether the body-wide observer is worth keeping alive.
		 *
		 * A surface belongs here when React can drop what this plugin wrote. The
		 * favicon does — its `<link>` lives in the head and is recreated on some
		 * navigations. The hero copy and tagline do, being body content.
		 *
		 * The tab title does not: `document.title` is owned by the shell and never
		 * removed by a re-render, so the title rewrite rides along in every sync
		 * pass without justifying a MutationObserver on its own. Listing it here
		 * would start a body-wide observer on every page nothing else needed one.
		 */
		...
		const ZH = {
			...
			"browser.hint": "留空则使用内置的长海图标与「长海智能」标题；标题会替换掉标签页里的官方产品名，其余部分（如会话名）保持不变。",
			"footer.hint":
		...
		const EN = {
			...
			"browser.hint": "Left empty, the tab icon and title fall back to the built-in Changhai mark and \"长海智能\". The title replaces the shipped product name and leaves the rest (such as the session name) alone.",
			"footer.hint":
