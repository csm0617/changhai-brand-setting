 ...
 * changes:
 *
 *   - a state file at `$DSH_HOME/custom-brand.json`, written atomically
 *     (tmp + rename), owner-only on POSIX;
 *   - a fenced JSON API at `/custom-brand/api` — POST `{method:"get"}` returns
...
const STATE_FILENAME = "custom-brand.json";
/** Route prefix owned by this plugin. */
const API_PREFIX = "/custom-brand/api";
...
function assertTrustedAuthority(entry) {
  ...
  throw new Error(
    `dsh-custom-brand: trustedHosts entry ${JSON.stringify(entry)} is not a bare host[:port] authority`,
  );
}
...
export function apply(ctx) {
  ...
  const clientRow = () => {
    try {
      ...
      const row = graph.entries.find((entry) => entry.id === "dsh-custom-brand");
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            console.error("[dsh-custom-brand] persistence API error:", error);
            writeJson(res, 500, { ok: false, error: { code: "internal", message: "internal error" } });
          }
        },
      }),
    "dsh-custom-brand: persistence API route",
  );
}
