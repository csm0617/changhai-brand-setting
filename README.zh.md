...
在设置里新增一个**品牌**页，把 DeepSeek Harness Web GUI 换成你自己的品牌：侧栏标志与名称、空会话首页标题、小标语与徽标、浏览器标签页标题与站点图标。总开关**默认开启**，装完即可见；关掉即立刻恢复官方品牌。
...
标志**自带图**——长海波浪，以 PNG data URL 内嵌在浏览器半里——所以 `logoKind` 默认就是 `image`，而总开关又默认开启，新装环境立刻就有品牌标志，无需随包携带任何文件。用户选图即替换它；选「官方」则把槽位交还官方占用者。内嵌的是原图的 50×51px，因为图像处理链**从不放大**（见 `MAX_IMAGE_EDGE`），在这里重采样只会让包变大、画质更差。
...
**站点图标**同样默认使用内嵌的长海图，所以标签页开箱即带品牌。它就是标志用的那张图，不额外增加体积；清空字段会回到它，而不是回到官方图标；选图即替换。标签页**标题**也自带文案（`长海智能`）：字段留空即用它，官方产品名随之离开标签页；会话名等其余部分由官方维护，本插件不碰。改写是**幂等**的——先撤销自己上一次的写入再重新替换，否则第二次编辑会因为产品名早已被替换掉而找不到可替换的文本，标签页便卡在旧值上。
...
- **启用自定义品牌**：总开关。**默认开启**，装上即带品牌（侧栏与首页标志、站点图标都是内置的长海图，首页带内置标语），无需先去设置里点一下。关闭后官方品牌立刻完全恢复，配置会保留。
...
- **浏览器**：标签页标题（在 `document.title` 里替换产品名，保留会话标题部分；留空即用内置的 `长海智能`）与站点图标；不选图时站点图标用内置的长海图。
...
冒烟测试通过捕获 `window.__ModuleLoader__.load` 加载 `lib/client.js`，用仿造的 cordis 上下文、DOM 与存储驱动它，断言：设置页已注册、槽位只在需要时被占用（关闭后释放）、每个已注册组件都能渲染、新装环境下标志即显示内置图且可被选图替换、站点图标同样回落到这张内置图、标签页标题替换生效、小标语在零配置下即显示内置句且仅在显式隐藏时移除、重复同步时复用同一节点而不重复创建，以及「与标题对齐」会写出实测的内缩量且从不改写标题的样式。React 依次从 `$DSH_WEB_MODULES`、`$DSH_HOME/profiles/web/node_modules`、`./node_modules` 解析。
...
- **切换会话后标签页标题可能短暂回落为官方产品名**：外壳在每次会话标题变化时都会整条重写 `document.title`（`会话名 — 产品名`），而这次赋值**不产生任何 DOM 变更**，本插件的 `MutationObserver` 收不到，于是要等到下一次同步（改设置、DOM 变化、缩放窗口）才会重新替换。属于观感问题而非错误：显示的仍是外壳的真实标题。彻底解决需要劫持 `document.title` 的访问器——那是猴子补丁，会与外壳及第三方插件争抢同一个原生属性，故不做。
