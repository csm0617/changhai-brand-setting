...
冒烟测试通过捕获 `window.__ModuleLoader__.load` 加载 `lib/client.js`，用仿造的 cordis 上下文、DOM 与存储驱动它，断言：设置页已注册、槽位只在需要时被占用（关闭后释放）、每个已注册组件都能渲染、标签页标题替换生效、小标语在零配置下即显示内置句且仅在显式隐藏时移除、重复同步时复用同一节点而不重复创建，以及「与标题对齐」会写出实测的内缩量且从不改写标题的样式。React 依次从 `$DSH_WEB_MODULES`、`$DSH_HOME/profiles/web/node_modules`、`./node_modules` 解析。
