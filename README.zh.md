...
**站点图标**同样默认使用内嵌的长海图，所以标签页开箱即带品牌。它就是标志用的那张图，不额外增加体积；清空字段会回到它，而不是回到官方图标；选图即替换。标签页**标题**保持原行为——字段留空时维持官方不变，因为它没有合理的默认文案。
...
- **浏览器**：标签页标题（在 `document.title` 里替换产品名，保留会话标题部分）与站点图标；不选图时站点图标用内置的长海图。
